...
const addPosts = (post) => {
  const postTemplate = `
    <li id="${post.data().userId}" data-template class="post-container">
      ...
      <div id=${post.id}>
        <textarea disabled class="user-post" rows="4" cols="50">${post.data().text}</textarea>
        ...
        <div data-edit class="edit-container display-none">
          ...
          <div class="edit-buttons">
            <button data-cancel="${post.id}" class="manage-post-btn cancel-btn" disabled>Cancelar</button>
            <button data-save="${post.id}" class="manage-post-btn save-btn" disabled>Salvar</button>
          </div>
        </div>
      </div>
     </li>
